...
import {
  PrintClientType,
  GetPrintersRequest,
  JingdongGetPrintersRequest,
  GetPrintersResult,
  JingdongGetPrintersResult,
  JingdongPrintRequest,
  JingdongNotifyPrintResult,
  PrintResponse,
  NotifyPrintResult,
  PrintRequest,
  GetPrintersParams,
  ...
  WaybillDocument,
  PrintDocument,
  JingdongPrintDocument,
  DoudianPrintDocument,
  KuaishouPrintDocument,
  PinduoduoPrintDocument,
  CainiaoPrintDocument,
  CainiaoPrintContent,
  DoudianPrintContent,
...
class PrintService {
  public _sockets: { [key: string]: WebSocket | undefined } = {};
  ...
  public connect(type: PrintClientType) {
    let socket = this._sockets[type];
    if (!socket) { socket = this.connectWebSocket(type); }
  }
  ...
  private connectWebSocket(
    ...
    socket.onopen = (ev: Event) => {
      ...
      this._eventBus.emitOpen(type, { requestID: 'ws_open', msg: `${name}打印控件已打开!` });
    };
  ...
  private registerEventBus_getPrinters(requestID: string, params: GetPrintersParams) {
    ...
    let offError = this._eventBus.onError(type, errorFun);
    let offGetPrinters = this._eventBus.onGetPrinters(type, getPrintersFun);
    let offEventBus = () => {
      offError();
      offGetPrinters();
    };
  ...
  private registerEventBus_print(requestID: string, params: PrintParams) {
              ...
              if (_data?.success == 'true') {
                ...
                if (errors.length > 0 || detail.length > 0) {
                  ...
                  for (let index = 0; index < documents.length; index++) {
                    const { documentID } = documents[index] || {};
                    const msg =
                      errors?.find((s) => s.seq == `${index + 1}`)?.msg ??
                      detail?.find((s) => s.seq == `${index + 1}`)?.msg ??
                      _data?.detailinfo?.info ??
                      '打印失败';
    ...
    let offError = this._eventBus.onError(type, errorFun);
    let offPrint = this._eventBus.onPrint(type, printFun);
    let offNotifyPrintResult = this._eventBus.onNotifyPrintResult(type, notifyPrintResultFun);
    let offEventBus = () => {
      offError();
      offPrint();
      offNotifyPrintResult();
    };
  }
  ...
  private parseDocumentID(documentID: string, index: number, total: number): string {
    if (documentID) {
      return `${documentID}_${index}/${total}_`;
    }
    return documentID;
  }
